...
* **Implementation**: Replaces random/OS-dependent semaphore wake-up order with a parent-managed scheduler. Each node has a waiting queue in shared memory. When a traveler reaches a busy node, it enters that node queue and blocks on a private semaphore. The parent process selects the next traveler according to the chosen scheduling algorithm and wakes only that traveler.
* **Algorithms**:
  * `fcfs` - First Come First Served: the first traveler that reached the node enters first.
  * `sjf` - Shortest Job First: the traveler with the shortest remaining route work enters first.
* **Build**: `make milestone7`
* **Run FCFS**: `./sim -schd fcfs sample_scheduler.txt`
* **Run SJF**: `./sim -schd sjf sample_scheduler.txt`
* **Comparison**: FCFS is fair and preserves arrival order, but a short traveler may wait behind a longer trip. SJF can reduce the waiting time of shorter trips because it chooses the traveler with the shortest remaining route first, but it may delay longer trips when many short trips arrive. The GUI title clearly shows which scheduler is active.

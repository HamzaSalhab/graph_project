...
// The IPC Payload Structure passed through the pipes
typedef struct {
    int traveler_id;
    int current_node;
    bool arrived;
...
// Tracker for parent process UI state
typedef struct {
    int current_node;
    bool arrived;
...
// Autonomous Child Behavior Logic
void run_child_behavior(int id, Traveler traveler, const Graph *graph, int write_fd) {
    // 1. Independent Path Calculation via Dijkstra inside Child Process space
    Path my_path = dijkstra_shortest_path(graph, traveler.source, traveler.dest);

    if (!my_path.found || my_path.length == 0) {
        // Stream back an immediate structural arrival flag if no route exists
        PositionUpdate update = { .traveler_id = id, .current_node = traveler.source, .arrived = true };
        write(write_fd, &update, sizeof(PositionUpdate));
        free_path(&my_path);
        return;
    }

    // 2. Walk path nodes independently and stream real-time telemetry updates to parent
    for (int i = 0; i < my_path.length; i++) {
        if (i > 0) {
            int weight = edge_weight_between(graph, my_path.vertices[i-1], my_path.vertices[i]);
            // Simulate stepping time delays proportional to edge cost
            usleep(weight * 250000);
        }

        PositionUpdate update;
        update.traveler_id = id;
        update.current_node = my_path.vertices[i];
        update.arrived = (i == my_path.length - 1);

        // Send payload via IPC Pipe channel
        write(write_fd, &update, sizeof(PositionUpdate));
    }
...
int main(int argc, char **argv) {
    ...
    // Phase 1: Call Extended Parsing Engine to extract graphs and multiple travelers
    ...
    // Set up local storage arrays for Parent process process hooks
    ...
    // Initialize state models for our visual rendering track
    for (int i = 0; i < num_travelers; i++) {
        visual_travelers[i].current_node = travelers[i].source;
        visual_travelers[i].arrived = false;
        visual_travelers[i].visual_position = node_positions[travelers[i].source];
    }

    // Phase 2: Inter-Process Communication Channel Initialization and Forking Loop
    for (int i = 0; i < num_travelers; i++) {
        int fd[2];
        if (pipe(fd) == -1) {
            perror("IPC Pipe construction crash");
            return 1;
        }
        ...
        if (pid == 0) {
            ...
            close(fd[0]); // Close unused read channel inside child

            run_child_behavior(i, travelers[i], &graph, fd[1]);

            close(fd[1]); // Close write socket before teardown
            free_graph(&graph);
            free(travelers);
            exit(0); // Safely terminate child context
        } else {
            // --- PARENT PROCESS CONTEXT ---
            close(fd[1]); // Close unused write channel inside parent
            parent_read_fds[i] = fd[0]; // Retain reading descriptor handle

            // Reconfigure reading pipes to non-blocking mode to keep rendering loop smooth
            int flags = fcntl(parent_read_fds[i], F_GETFL, 0);
            fcntl(parent_read_fds[i], F_SETFL, flags | O_NONBLOCK);
        }
    }

    // Phase 3: Parent Graphic Context Window Assembly
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "OS Multi-Traveler IPC Graph Simulation");
    SetTargetFPS(60);

    while (!WindowShouldClose()) {
        // Scan for pending IPC telemetry payloads from all running child threads
        for (int i = 0; i < num_travelers; i++) {
            if (visual_travelers[i].arrived) continue;

            PositionUpdate update;
            ssize_t bytes = read(parent_read_fds[i], &update, sizeof(PositionUpdate));

            if (bytes == sizeof(PositionUpdate)) {
                // IPC Packet Successfully intercepted. Map downstream state arrays
                visual_travelers[update.traveler_id].current_node = update.current_node;
                visual_travelers[update.traveler_id].arrived = update.arrived;
            } else if (bytes < 0 && (errno != EAGAIN && errno != EWOULDBLOCK)) {
                // Read anomaly handling
            }
        }

        // Apply smooth visual interpolation toward current objective positions
        for (int i = 0; i < num_travelers; i++) {
            Vector2 target_pos = node_positions[visual_travelers[i].current_node];
            visual_travelers[i].visual_position = lerp_vec(visual_travelers[i].visual_position, target_pos, 0.08f);
        }

        // Graphic Framework Processing Frame Call
        BeginDrawing();
        ClearBackground(RAYWHITE);

        DrawText("Milestone 5: Decentralized Child IPC Architecture", 170, 25, 26, MAROON);
        draw_graph(&graph, node_positions);

        // Blit active multi-traveler nodes dynamically across layout coordinates
        for (int i = 0; i < num_travelers; i++) {
            // Generate color hashes unique to each specific client ID tracker
            Color runner_color = GetColor(0x2080A0FF + (i * 0x3A2C1F00));
            ...
            // Label individual traveler identifiers above current localized node tracking
    ...
    // Phase 4: Resource Cleanup & Zombie Process Collection
    CloseWindow();
